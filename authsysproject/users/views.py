...
def client_dashboard(request):
    ...
    if current_user_personal_info.institution_name:
        institution_name = current_user_personal_info.institution_name.strip().lower()
        print("Normalized Institution Name:", institution_name)

        # Fetch and sort PDFs by patient_id
        pdfs = XrayReport.objects.filter(institution_name__iexact=institution_name).order_by('patient_id', '-id')
        ...
        for patient_id, group in grouped_pdfs:
            ...
            dicom_data = DICOMData.objects.filter(
                patient_id=patient_id,
                patient_name__iexact=normalized_name,
                twostepcheck=False
            ).first()
